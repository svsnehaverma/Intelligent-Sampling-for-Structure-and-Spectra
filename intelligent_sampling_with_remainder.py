...
def parse_args(args):
    ...
    p.add_argument('choice', type=str, help=help_strs['xyz'])
...
def intelligent_sample(xyz, xanes, min, max, step):
    ...
    for n in n_sample_list:
        ...
        if n > 300000:
            remainder_folder = n % 300000
            Path(f'./IT_data/{remainder_folder}/xyz').mkdir(parents=True, exist_ok=True)        
        ...
        for file in xyz.iterdir():
            if file.stem + '.xyz' in ids_to_keep:
                ...
                if n > 300000:
                    shutil.copy(file, f'./IT_data/{remainder_folder}/xyz')
...
def main(args):
    ...
    if args.choice == 'xyz':
        ...
        print("Invalid choice. Please choose 'xyz' for XYZ sampling.")
